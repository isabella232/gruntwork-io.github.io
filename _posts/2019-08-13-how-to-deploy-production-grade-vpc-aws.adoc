...
https://aws.amazon.com/transit-gateway/[AWS Transit Gateway] or
https://docs.aws.amazon.com/vpc/latest/userguide/vpc-sharing.html[Shared VPCs].
====
...
Interface endpoints::
  This is a new type of endpoint that is backed by https://aws.amazon.com/privatelink/[PrivateLink], which is
...
// embed examples from our Ref Arch code? (Rho:) Yes.

The first step is to deploy a management VPC for DevOps tooling, such as a CI server or a bastion host. To do this, you
can use the https://github.com/gruntwork-io/terraform-aws-vpc/tree/master/modules/vpc-mgmt[vpc-mgmt] module from the
Gruntwork Infrastructure as Code Library.

[.exceptional]
IMPORTANT: You must be a [js-subscribe-cta]#Gruntwork subscriber# to access `terraform-aws-vpc`.
...
SonarQube), then use https://github.com/gruntwork-io/terraform-aws-vpc/tree/master/modules/vpc-app[vpc-app] instead, which has
...
terraform {
  # TODO: this didn't work for me. after creating the region/account/common/terragrunt hcl files, it complained that
  # backend was defined more than once.
  # The configuration for this backend will be filled in by Terragrunt or via a backend.hcl file. See
  # https://www.terraform.io/docs/backends/config.html#partial-configuration
  backend "s3" {}

  # TODO: bump this to a higher version
...
# TODO: We say to replace version here, but we didn't give that advice in the Landing Zone guides. Should add it there.
...
with the latest version from the https://github.com/gruntwork-io/terraform-aws-vpc/releases[releases page]:
...
module "vpc" {
  ...
  source = "git::git@github.com:gruntwork-io/terraform-aws-vpc.git//modules/vpc-mgmt?ref=<VERSION>"
...
# TODO: This pattern of linking to the actual code in a repo for the larger files, we should adopt this in the
# landing zone guide!
...
module "vpc_network_acls" {
  # Make sure to replace <VERSION> in this URL with the latest terraform-aws-vpc release
  # TODO: this is wrong!
  source = "git::git@github.com:gruntwork-io/terraform-aws-vpc.git//modules/vpc-mgmt-network-acls?ref=<VERSION>"
...
# TODO: again, this is not good advice. we need to mention they should only use `plan`, not `apply` for testing, and give them a much better
# idea of exactly what they need to set. I think they should just create the terragrunt.hcl and run plan against that, which is explained further on,
# So we should have them create the terragrunt.hcl, then run plan, git tag/push, and then run apply.
At this point, you'll want to test your code. See link:/guides/foundations/how-to-use-gruntwork-infrastructure-as-code-library#manual_tests_terraform[Manual tests for Terraform code]
and link:/guides/foundations/how-to-use-gruntwork-infrastructure-as-code-library#automated_tests_terraform[Automated tests for Terraform code]
for instructions.
...
----
terraform {
  source = "git::git@github.com:<YOUR_ORG>/infrastructure-modules.git//networking/vpc-mgmt?ref=v0.3.0"
}
----
+
...
# TODO: kms_key_user_iam_arns is missing here
.infrastructure-live/production/us-east-2/prod/networking/vpc-mgmt/terragrunt.hcl
[source,hcl]
----
inputs = {
  ...
  num_nat_gateways = 3
  kms_key_user_iam_arns = [<YOUR_USER_ARNS>]
}
----

# TODO: I think this should be placed above inputs, so that locals can also be used.
+
...
Deploy the VPC by running `terragrunt apply`. Make sure to authenticate into the AWS account.
...
# TODO: Should mention that you need to save all the outputs somewhere for use later!
...
except this time, you'll be using the https://github.com/gruntwork-io/terraform-aws-vpc/tree/master/modules/vpc-app[vpc-app]
...
----
module "vpc_network_acls" {
  source = "git::git@github.com:gruntwork-io/terraform-aws-vpc.git//modules/vpc-app-network-acls?ref=<VERSION>"
...
data "terraform_remote_state" "mgmt_vpc" {
  ...
  config = {
    ...
    # TODO: the key didn't work for me. this is what I had to use.
    # Based on the folder structure we recommend in [configure_wrapper_module] (management)
    key = "prod/us-west-1/prod/networking/vpc-mgmt/terraform.tfstate"

  }
}
----

You can then use the https://github.com/gruntwork-io/terraform-aws-vpc/tree/master/modules/vpc-peering[vpc-peering] module to
...
module "mgmt_vpc_peering_connection" {
  source = "git::git@github.com:gruntwork-io/terraform-aws-vpc.git//modules/vpc-peering?ref=<VERSION>"
...
# TODO: The outputs include `dns_mgmt_to_app` but that module is not defined for the main.tf above. Since the outputs are linked to main
branch, and main.tf is hardcoded, there's a mismatch in versions.
# it's here: https://github.com/gruntwork-io/infrastructure-modules-multi-account-acme/blob/master/networking/vpc-app/main.tf#L183-L218

You'll also need to create a DNS forwarder between the management VPC and the app VPC:

.infrastructure-modules/networking/vpc-app/main.tf
[source,hcl]
----
module "dns_mgmt_to_app" {
  source           = "git::git@github.com:gruntwork-io/terraform-aws-vpc.git//modules/vpc-dns-forwarder?ref=<VERSION>"
...
By default, `var.create_dns_forwarder` is `false`, so you won't actually create a DNS forwarder. We include it here because the
outputs, which follow, depend on this module being defined.
...
----
terraform {
  source = "git::git@github.com:<YOUR_ORG>/infrastructure-modules.git//networking/vpc-app?ref=v0.4.0"
}
----
+
. *Set the variables for the VPC in that environment.* Namely, set a unique VPC name and CIDR block for each VPC, and
...
inputs = {
  ...
  num_nat_gateways = 3
  kms_key_user_iam_arns = [<YOUR_USER_ARNS>]
# TODO: we also need to add terraform_state_s3_bucket and terraform_state_aws_region
}
----
+
...
Deploy the VPC by running `terragrunt apply`, authenticating to your AWS account.
...
# TODO: we should tell people to keep a note of the outputs from this!
