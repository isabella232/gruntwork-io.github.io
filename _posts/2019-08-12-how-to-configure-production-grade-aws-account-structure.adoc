...
terraform {
  ...
  # TODO: Totally remove this! Just have them create the root terragrunt.hcl to define the bucket stuff.
  backend "s3" {}
...
variable "users" {
  ...
  # Example:
  # TODO: shouldn't this be called `users = { ... }`? The implication is that they won't modify this variables.tf, but rather set the users in their testing/terraform.tfvars file.
  # So in that file, it should be named `users`, not `default`.
  # default = {
  #   alice = {
...
# TODO: We should also detail exactly how to set up your backend configuration, including:
 - terragrunt.hcl at the root:
    ...
    remote_state {
      ...
      config = {
        encrypt        = true
        bucket         = "rho-refarchdeeppdive-state=${local.account_name}-${local.aws_region}"
 ...
 - common.hcl also at the root:
    ...
    ----
 - in each <account_name>/_global/ folder, a region.hcl:
    [source,hcl]
    ----
...
#TODO: need to flesh these instructions out...
At this point you can test your changes by running `plan`.
...
# TODO: maybe the above works, to use `import` to put the user under Terraform management, but it was such a long frickin' section that I decided to skip the import. Instead, I used aws-vault to run the first terraform apply using the root account user, and then changed it to use the root IAM user once they were created by Terraform.
...
# TODO: Wow. This makes no sense at all. How the heck do I do this? Can't you just give me the code I'm supposed to add and where it goes?
Luckily I read through the "testing" links that mentioned a backend.hcl... But we really shouldn't be giving that as advice here because
attempting to test really screwed me up. Who knows how many accounts I'm going to end up with in this organization. I kinda want to start over
...
# TODO: this is fine to mention here, but in my flow, because I followed the advice to test the code first, I had to do this
well ahead of time. If we remove that directive to test, we should be fine here.
...
# TODO: Instead, talk here about how to write your infrastructure-live/common.hcl, and create variables for these outputs that you can use as locals in your other hcl files.
...
# TODO: remove this directive! Don't test this. If you do, just do a plan test. Also we should provide all the necessary information for this.
# TODO: We should also detail exactly how to set up your backend.hcl, or a configuration for the state s3 bucket.
...
# TODO: remove this, or explain in detail exactly how to test, don't just link to this page, it will confuse folks.
At this point, you'll want to test your code. See
link:/guides/foundations/how-to-use-gruntwork-infrastructure-as-code-library#manual_tests_terraform[Manual tests for Terraform code] and
link:/guides/foundations/how-to-use-gruntwork-infrastructure-as-code-library#automated_tests_terraform[Automated tests for Terraform code]
for instructions.
...
accounts (i.e., dev, stage, prod, and shared-services, plus sandbox and testing accounts you might have) as
...
inputs = {
  ...
  # TODO: Is this correct/needed for the shared-services account?
...
# TODO: Does it matter what order things are in? Where we give this advice to configure the backend, probably it's better
to tell people order the blocks like: terraform, include, locals, and finally inputs.
Configure your Terraform backend:
