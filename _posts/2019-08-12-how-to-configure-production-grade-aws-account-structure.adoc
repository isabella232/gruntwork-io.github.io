...
terraform {
  ...
  # TODO: should mention that they need to create the bucket first!
  backend "s3" {}
...
for instructions.
# TODO: These links specify creating a testing/backend.hcl and testing/terraform.tfvars containing their variables. I think we need to mention
that they'll have to insert variables into their terraform.tfvars, beyond what's mentioned in the links above. Might be worth stating what they need to
set here, directly, like:

[source,hcl]
----
aws_region     = "us-west-1"
aws_account_id = "453687634329"
name_prefix    = "rho-testing-refarchdeepdive"
child_accounts = {
  logs = {
    email           = "rho+logs-account-1@gruntwork.io"
      is_logs_account = true
  }
  security = {
    email                      = "rho+security-account-1@gruntwork.io"
      role_name                  = "OrganizationAccountAccessRole"
      iam_user_access_to_billing = "DENY"
      tags = {
        Tag-Key = "tag-value"
...
# TODO: Actually, in order to test, you'd have to be able to run `terraform destroy`, which isn't possible on accounts. So this advice is actually really bad.
We should remove the advice to test your module that wraps account-baseline-root!


# TODO: We shouldn't encourage testing here, so remove this block, or move it to after you run the terragrunt command.
...
# TODO: The only way I could figure out how to do this was not to include the `include` block here, because I had nothing in parent folders. And this tutorial doesn't say to have anything there either.
