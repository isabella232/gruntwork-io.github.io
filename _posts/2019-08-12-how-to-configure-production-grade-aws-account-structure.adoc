...
# TODO: one of the pre-requisites should DEFINITELY be, to create infra-live and infra-modules repos, and to pay careful attention
to which repo we're modifying as we walk through the process.
...
terraform {
  ...
  # TODO: do we have support for terraform 13 or 14 now?
...
# TODO: We need to wordwrap here, I think, because on my screen, the code box this ends up in has a horizontal scrollbar which makes it frustrating to read all the text. We definitely want people to read! In fact, if I had, I might have realized some things I didn't do correctly. On my screen the window is 85 columns wide.
...
inputs = {
  ...
  iam_groups_for_cross_account_access = [
    ...
    # TODO: need to be explicit about exactly how to duplicate these. it's not clear if every instance of `dev` should be replaced or just the account ids, or what.
  ]
...
# TODO: Everywhere we give this advice to configure the backend, probably it's better to tell people organize this
differently in their hcl files. First terraform block, then include, locals, and finally inputs.
Configure your Terraform backend:
