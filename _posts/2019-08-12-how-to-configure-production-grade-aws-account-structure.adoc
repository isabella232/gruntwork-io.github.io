...
Shared account::
  The _shared account_ is used for infrastructure and data that is shared amongst all the application
  accounts, such as CI servers and artifact repositories. For example, in your shared account, you might use
  https://aws.amazon.com/ecr/[ECR] to store Docker images and Jenkins to deploy those Docker images to dev, stage, and
  prod. Since the shared account may provide resources to (e.g., application packages) and has access to
...
Logs account::
  ...
  accounts, shared, etc.—will send their AWS Config and CloudTrail data to this account so that you have a
...
for each business unit, and within each Organization Unit, you create a set of application, shared, sandbox,
...
allow-auto-deploy-access-from-other-accounts::
  ...
  server in the shared account, and gives that server permissions to deploy EC2 Instances and Auto Scaling
...
Code repositories::
  You will need to initialize two repos: `infrastructure-modules` and `infrastructure-live`. Pay careful attention to
  which one you are modifying as you go through this guide.
...
terraform {
  # Partially configure the backend. It will be filled in by Terragrunt if a root level terragrunt.hcl defines
  # remote_state. See https://www.terraform.io/docs/backends/config.html#partial-configuration
  backend "s3" {}
...
variable "users" {
  ...
  # default = {
  #   alice = {
  #     groups = ["iam-user-self-mgmt", "developers", "ssh-grunt-sudo-users"]
  #   }
  #
  #   bob = {
  #     path   = "/"
  #     groups = ["iam-user-self-mgmt", "ops", "admins"]
  #     tags   = {
  #       foo = "bar"
  #     }
  #   }
  #
  #   carol = {
  #     groups               = ["iam-user-self-mgmt", "developers", "ssh-users"]
...
At this point you can test your changes by running `terragrunt plan`.
...
==== Create reusable variables across modules and terragrunt files
...
Now you're going to make some HCL files that store variables to be used across your modules. _You won't be able to fill everything out just yet._ Your AWS account IDs
will be generated after applying the account-baseline-root to the root account. At that point you can update these files. Create them now to have them ready to use.

Create a `terragrunt.hcl` at the root of your infrastructure-live repo. As you can see, it references `common`, `account`, and `region` hcl files which we'll create shortly.
...
# TODO: Replace with your organization name!
      bucket         = "${local.name_prefix}-${local.account_name}-${local.aws_region}"
...
Also create a `common.hcl` file at the root of your infrastructure-live repo.

[source,hcl]
----
locals {
  # Default AWS region for API calls
  # TODO: Replace with your default region!
  default_region = "us-west-1"

  # A prefix used for naming resources
  # TODO: Replace with your organization name!
  name_prefix = "<YOUR_ORGANIZATION_NAME>"

  # Centrally define all our AWS account IDs
  # TODO: Replace with your account IDs!
  account_ids = {
    root     = "<YOUR_ROOT_ACCOUNT_ID>"
    security = "<YOUR_SECURITY_ACCOUNT_ID>"
    shared   = "<YOUR_SHARED_SERVICES_ACCOUNT_ID>"
    dev      = "<YOUR_DEV_ACCOUNT_ID>"
    stage    = "<YOUR_STAGE_ACCOUNT_ID>"
    prod     = "<YOUR_PROD_ACCOUNT_ID>"
    logs     = "<YOUR_LOGS_ACCOUNT_ID>"
  }
}
----

Create a very simple `account.hcl` at the root of each `<account_name>/` folder. For example, in
the `infrastructure-live/dev/account.hcl`, create:
[source, hcl]
----
...
Be sure to match the `account_name` with the actual account name (`dev`, `stage`, `prod`, `shared`, `logs`, `security`).


Now in each `<account_name>/_global/` folder, a `region.hcl` file.
...
Do the same in each `<account_name>/<region>/` folder. This `region.hcl` file is a bit different.
...
locals {
    ...
    # TODO: Update to use your default region here!
...
----
terraform {
  source = "git::git@github.com:<YOUR_ORG>/infrastructure-modules.git//modules/account-baseline-root?ref=v0.3.0"
...
Take note of all of this data. In fact, it's a good time to update your root-level
`common.hcl` to include these account IDs.
...
other accounts. To do this, create a new module called `account-baseline-app`  in your `infrastructure-modules` repo:
...
variable "name_prefix" {
  description = "The name used to prefix AWS Config and CloudTrail resources,
    including the S3 bucket names and SNS topics used for each."
  type        = string
}

variable "aws_region" {
  description = "The AWS Region to use as the global config recorder and seed region
    for GuardDuty."
  type        = string
}

variable "aws_account_id" {
  description = "The AWS Account ID the template should be operated on. This avoids
...
At this point, you'll want to test your code. To do this without deploying resources, run `terragrunt plan`. Make sure the plan output looks sane before proceeding.

If you have time to write more robust testing, also see
...
At this point, you'll want to test your code. Run `terraform plan` to do plan testing, make sure there are no errors and the output looks sane.

For more robust testing, also see
link:/guides/foundations/how-to-use-gruntwork-infrastructure-as-code-library#manual_tests_terraform[Manual tests for Terraform code] and
link:/guides/foundations/how-to-use-gruntwork-infrastructure-as-code-library#automated_tests_terraform[Automated tests for Terraform code]
for instructions.
  ...
  iam_groups_for_cross_account_access = [
    {
      ...
      iam_role_arns = ["arn:aws:iam::<STAGE_ACCOUNT_ID>:role/allow-full-access-from-other-accounts"]
    },
    {
      group_name    = "_account.stage-read-only",
      iam_role_arns = ["arn:aws:iam::<STAGE_ACCOUNT_ID>:role/allow-read-only-access-from-other-accounts"]
    },
    {
      group_name    = "_account.stage-dev",
      iam_role_arns = ["arn:aws:iam::<STAGE_ACCOUNT_ID>:role/allow-dev-access-from-other-accounts"]
    },
    {
      group_name    = "_account.stage-openvpn-admins",
      iam_role_arns = ["arn:aws:iam::<STAGE_ACCOUNT_ID>:role/openvpn-allow-certificate-revocations-for-external-accounts"]
    },
    {
      group_name    = "_account.stage-openvpn-users",
      iam_role_arns = ["arn:aws:iam::<STAGE_ACCOUNT_ID>:role/openvpn-allow-certificate-requests-for-external-accounts"]
    },
...
. **Create IAM groups**. We've created IAM groups, both for permissions within the security account (e.g.,
  ...
  fill out the section for permissions in dev, prod, logs, and shared accounts.
...
inputs = {
  ...
  # Allow a CI server in the shared account to assume the auto-deploy IAM role in this account
...
. **Configure the Auto Deploy IAM role**. We also create an `auto-deploy` IAM role that can be assumed by a CI server
  in the shared account to do deployments. This role will have the permissions specified in
